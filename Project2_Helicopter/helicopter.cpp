...
void Animate()
{
	...
	if (!Frozen)
	{
		...
		Time = (float)ms / (float)MS_IN_THE_ANIMATION_CYCLE; // [ 0., 1. )s
		rotationAngle += Time * 50;
	}
...
void Display()
{
	...
	{
		Reset();
		// inside view from the cokpit, we only see the blade and some green around and we see the object
	...
	glRotatef(3 * rotationAngle, 0., 0., 1.);
	glScalef(1.5, 1, 2);
	...
	glLoadIdentity();
	glColor3f(1., 1., 1.);
	DoRasterString(5., 5., 0., "Masson Margaux - Helicopter");
...
void InitLists()
{
	...
	// Teapot - 3D scene in front the helicopter
...
void MouseMotion(int x, int y)
{
	...
	if ((ActiveButton & LEFT) != 0 & view == 0)
	{
		Xrot += (ANGFACT * dy);
		Yrot += (ANGFACT * dx);
	}

	if ((ActiveButton & MIDDLE) != 0 & view == 0)
	{
